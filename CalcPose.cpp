...
bool calcPose(const std::vector<Eigen::Vector3d>& objPoints, const std::vector<Eigen::Vector2d>& imgPoints, Eigen::Isometry3d& current_pose)
{
	...
	for (int i = 0; i < points_num; i++)
	{
		...
		object_point_average += Eigen::Vector3d(X, Y ,Z);
		detected_point_average += Eigen::Vector2d(x, y);
	}
	...
	Eigen::JacobiSVD<Eigen::Matrix<double, 12, 12>> svd(AtA, Eigen::ComputeFullU | Eigen::ComputeFullV);
	int min_sigularIndex = 0;
	Eigen::VectorXd sigular_valueVec = svd.singularValues();
	for (int i = 0; i < 12; i++)
	{
	...
	sigma.setIdentity();
	sigma(2, 2) = (svd_R.matrixU() * svd_R.matrixV().transpose()).norm();
	...
	while (1) {
		...
		for (int i = 0; i < points_num; i++)
		{
			...
			projected_point(0) = P_cam(0) / P_cam(2);
			projected_point(1) = P_cam(1) / P_cam(2);
