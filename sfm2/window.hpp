...
protected:
	...
	void glPerspectiveCam(double focalLength, int viewWidth, int viewHeight, double nearPlane, double farPlane)
	{
		double fP = nearPlane / focalLength;

		double left = (-viewWidth / 2) * fP;
		double right = (viewWidth / 2) * fP;
		double bottom = (-viewHeight / 2) * fP;
		double top = (viewHeight / 2) * fP;
